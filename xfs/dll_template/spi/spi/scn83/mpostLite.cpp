...
std::vector<char> MpostLite::sendCommand(std::vector<char> &payloadIn)
{
	...
	Xfs::getInstance()->l.debug("Payload:", payloadIn.data(), payloadIn.size());
	...
	Xfs::getInstance()->l.debug("Response received:", resp, r);
	...
	Xfs::getInstance()->l.debug("Payload:", payload.data(), payload.size());
	

	Xfs::getInstance()->l.debug("Checksum resp:"+ std::to_string(checkSum(resp)));
	
	if (checkSum(resp) != resp[resp[1] - 1])
	{
		Xfs::getInstance()->l.debug("Wrong checksum" );
	}
	else
		Xfs::getInstance()->l.debug("Checksum ok");   
...
{
	Xfs::getInstance()->l.debug("Checking ACK for command");
...
{
	Xfs::getInstance()->l.debug("Device status changed, executing signal");
	changed();
}
...
{
	Xfs::getInstance()->l.debug("setExtendedPollResults called");
	if (r[1]== 0x02)
	{
		Xfs::getInstance()->l.debug("subtype = 0x02");
		if (!bankNoteInEscrow.isPresent)
		{
			statusChanged();
		}
		Xfs::getInstance()->l.debug("bankNoteInEscrow.isPresent = true");
...
bool MpostLite::setPollResults(char *data) 
{
	...
	if(ch)
		Xfs::getInstance()->l.debug("Device state:" + printStr);
	...
	if (ch)
		Xfs::getInstance()->l.debug("Device status:" + printStr);
	...
	if (ch)
		Xfs::getInstance()->l.debug("Misc state:" + printStr);

	if (ch)
	{
		printStr = "";
		if (model &(char)Model::SCAdvance83)printStr = "SCAdvance83";
		Xfs::getInstance()->l.debug("Model:" + printStr);

		Xfs::getInstance()->l.debug("SW Version:" + std::to_string(version));
	}
	return ch;
...
{
	Xfs::getInstance()->l.debug("configureNoteTypes");
...
{
	Xfs::getInstance()->l.debug("Enable accepting:");

	
	Xfs::getInstance()->l.debug("cmd1:" + std::to_string(cmd1.load()));
...
{
	Xfs::getInstance()->l.debug("return:");
...
{
	Xfs::getInstance()->l.debug("reset:");
	Xfs::getInstance()->l.debug("stopig poll");

	stopPoll();
	
	AutoLock locl(&sendCmdCs);

	Xfs::getInstance()->l.debug("Sending reset command:");
	std::vector<char>c = { 0x60, 0x7F, 0x7F, 0x7F };


	Xfs::getInstance()->l.debug("Payload:", c.data(), c.size());
	...
	Xfs::getInstance()->l.debug("starting poll");
...
{
	Xfs::getInstance()->l.debug("Disable accepting:");
...
{
	Xfs::getInstance()->l.debug("Sending querySoftwareCRC command:");
	std::vector<char>c = { 0x60, 0x00, 0x00, 0x00 };
	auto r = sendCommand(c);

	Xfs::getInstance()->l.debug("querySoftwareCRC data______:", r.data(), r.size());
	return;
}

void MpostLite::queryCashBoxTotal()
{
	Xfs::getInstance()->l.debug("Sending queryCashBoxTotal command:");
	std::vector<char>c = { 0x60, 0x00, 0x00, 0x01 };
	auto r = sendCommand(c);

	Xfs::getInstance()->l.debug("queryCashBoxTotal data______:", r.data(), r.size());
	return;
}

void MpostLite::queryVariantComponentName()
{
	Xfs::getInstance()->l.debug("Sending queryVariantComponentName command:");
	std::vector<char>c = { 0x60, 0x00, 0x00, CmdAuxQueryAcceptorVariantName};
	auto r = sendCommand(c);

	Xfs::getInstance()->l.debug("queryVariantComponentName data______:", r.data(), r.size());
	...
	for(auto ck:currencies)
		Xfs::getInstance()->l.debug("currency:"+std::string(ck));
...
{
	Xfs::getInstance()->l.debug("Sending clearCashBoxTotal command:");
	std::vector<char>c = { 0x60, 0x00, 0x00, CmdAuxClearCashBoxTotal };
	auto r = sendCommand(c);

	Xfs::getInstance()->l.debug("clearCashBoxTotal data______:", r.data(), r.size());
}

void MpostLite::queryValueTable()
{
	Xfs::getInstance()->l.debug("Sending queryValueTable command:");
	std::vector<char>c = { 0x70, 0x06, 0x00, 0x00,0x00 };
	auto r = sendCommand(c);
	Xfs::getInstance()->l.debug("queryValueTable data______:", r.data(), r.size());
	for (int i = 0; i < 7; i++)
	{
		int  idx = 8 + i * 10;
	...
	{
		Xfs::getInstance()->l.debug("note type:"+std::to_string(n.index) + ", iso="+n.iso[0]+ n.iso[1]+ n.iso[2]+", base value="+std::to_string(n.base)+ ", exp="+std::to_string(n.exp));
	}
