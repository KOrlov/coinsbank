...
void CIMSCN83::sendWFS_EXEE_CIM_INSERTITEMS(HSERVICE hService) 
{
	...
	Xfs::getInstance()->l.slog("sendWFS_EXEE_CIM_INSERTITEMS  EXECUTING");
	map<HWND, EventReg> events = getEventMap(hService, WFS_EXECUTE_EVENT);
	for (auto i : events)
	{
		LPWFSRESULT pResult;
...
{
	Xfs::getInstance()->l.slog("Executing start cash in");
	...
	Xfs::getInstance()->l.slog("Starting cash in");	

}
void CIMSCN83::execWFS_CMD_CIM_CASH_IN(REQUESTID reqId)
{
	Xfs::getInstance()->l.slog("execWFS_CMD_CIM_CASH_IN");
	...
	if (mpost->bankNoteInEscrow.isPresent)
	{
		...
		Xfs::getInstance()->l.slog("mpost->bankNoteInEscrow.noteType.base=" + std::to_string(mpost->bankNoteInEscrow.noteType.base));
		Xfs::getInstance()->l.slog("mpost->bankNoteInEscrow.noteType.exp=" + std::to_string(mpost->bankNoteInEscrow.noteType.exp));
		Xfs::getInstance()->l.slog("val=" + std::to_string(val));
		Xfs::getInstance()->l.slog("notetypes count=" + std::to_string(noteTypes.size()));
...
void CIMSCN83::execWFS_CMD_CIM_CASH_IN_END(REQUESTID reqId)
{
	...
	auto a = this->memAlloc->AllocateMore(sizeof(WFSCIMCASHINFO), requests[reqId]->pResult, (void **)&requests[reqId]->pResult->lpBuffer);
	Xfs::getInstance()->l.slog("constructLPWFSCIMCASHIN");
...
void CIMSCN83::execWFS_CMD_CIM_SET_CASH_UNIT_INFO(REQUESTID reqId)
{
	...
	Xfs::getInstance()->l.slog("execWFS_CMD_CIM_SET_CASH_UNIT_INFO");
...
{
	Xfs::getInstance()->l.slog("WFS_CMD_CIM_CONFIGURE_NOTETYPES");
...
void CIMSCN83::copyLpInput(LPVOID lpData, REQUESTID reqId)
{
	...
	if (requests[reqId]->messageType == MSG_EXECUTE && lpData)
	{
		...
		{
			Xfs::getInstance()->l.slog("WFS_CMD_CIM_SET_CASH_UNIT_INFO copy data");
		...
		if (requests[reqId]->cmd == WFS_CMD_CIM_CONFIGURE_NOTETYPES) 
		{
			...
			Xfs::getInstance()->l.slog("NUM OF NOTES="+std::to_string(c));
...
{	
	...
	Xfs::getInstance()->l.slog("CIMSCN83::CIMSCN83 executing, port="+this->port);
...
void CIMSCN83::getStoredData()
{
	...
	{
		Xfs::getInstance()->l.slog("note configured:"+std::to_string(i));
	}
