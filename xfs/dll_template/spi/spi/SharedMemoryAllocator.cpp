...
HRESULT SharedMemoryAllocator::AllocateBuffer(ULONG ulSize, LPVOID *lppvData)
{
	...
	Xfs::getInstance()->l.slog("AllocateBuffer  ulSize=" + std::to_string(ulSize) + ", hResult=" + std::to_string(result)+", pointer="+std::to_string((long)lppvData));
...
HRESULT SharedMemoryAllocator::AllocateMore(ULONG ulSize, LPVOID lpvOriginal, LPVOID * lppvData)
{
	...
	Xfs::getInstance()->l.slog("AllocateMore  ulSize=" + std::to_string(ulSize) +", hResult="+std::to_string(result));
