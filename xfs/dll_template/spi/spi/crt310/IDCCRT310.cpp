...
void IDCCRT310::copyLpInput(LPVOID lpData, REQUESTID reqId)
{
	...
	if (requests[reqId]->messageType == MSG_EXECUTE && lpData)
	{
		...
		{
			Xfs::getInstance()->l.debug("copyLpInput WFS_CMD_IDC_READ_RAW_DATA");
			...
			*lpwReadDatad = *lpwReadDatas;
			Xfs::getInstance()->l.debug("LPWORD created");

		}
		if (requests[reqId]->cmd == WFS_CMD_IDC_CHIP_IO)
		{
			Xfs::getInstance()->l.debug("copyLpInput WFS_CMD_IDC_CHIP_IO");
			...
			Xfs::getInstance()->l.debug("LPWFSIDCCHIPIO created");
...
void IDCCRT310::execWFS_CMD_IDC_READ_RAW_DATA(REQUESTID reqId)
{
	...
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_READ_RAW_DATA reqId=" + std::to_string(reqId) + ", lpwReadData=" + std::to_string(*lpwReadData));
	
	
	Xfs::getInstance()->l.debug("(*lpwReadData)&WFS_IDC_TRACK1" + std::to_string((*lpwReadData)&WFS_IDC_TRACK1));
	Xfs::getInstance()->l.debug("(*lpwReadData)&WFS_IDC_TRACK2" + std::to_string((*lpwReadData)&WFS_IDC_TRACK2));
	...
	{
		Xfs::getInstance()->l.debug("Status.fwDevice != WFS_IDC_DEVONLINE, faDevice=" + std::to_string(status.fwDevice));
	...
	{
		Xfs::getInstance()->l.debug("Media jammed, ...");
	...
	{
		Xfs::getInstance()->l.debug("Media not present, enabling card-in");
	...
	if (requests[reqId]->isCancelled)
	{
		...
		Xfs::getInstance()->l.debug("cancelled");


		Xfs::getInstance()->l.debug("Disabling card-in");

		int ec = CRT310_CardSetting(port.port, 0x1, 0x1);
		if (ec != 0)
		{
			GetErrCode(&ec);
			Xfs::getInstance()->l.debug("Error disabling card-in:" + std::to_string(ec));
	...
	Xfs::getInstance()->l.debug("MC_ReadTracks track cmd=" + std::to_string(track));
	...
	if (track)
	{
		...
		Xfs::getInstance()->l.debug("MC_ReadTracks ec=" + std::to_string(ec));
		if (ec != 0)
		{
			GetErrCode(&ec);
			Xfs::getInstance()->l.debug("Error reading tracks:" + std::to_string(ec));
			checkEc(ec);
			requests[reqId]->pResult->hResult = this->hResult;
			return;
		}
		Xfs::getInstance()->l.debug("Read tracks =", data, len);
		...
		Xfs::getInstance()->l.debug("Track 1 length=" + std::to_string(len1));
		...
		Xfs::getInstance()->l.debug("Track 2 length=" + std::to_string(len2));
		...
		Xfs::getInstance()->l.debug("Track 3 length=" + std::to_string(len3));
	...
	if ((*lpwReadData)&WFS_IDC_CHIP)
	{
		...
		{ 
			...
			Xfs::getInstance()->l.debug("Warm reset =" + std::to_string(ec));
			Xfs::getInstance()->l.debug("CPU_WarmReset ec=" + std::to_string(ec));
			Xfs::getInstance()->l.debug("CPU_WarmReset", dataAtr, lenAtr);

			if (ec)
			{
				if (ec == 0x4e)//Unsuccessful
				{
					Xfs::getInstance()->l.debug("Unsuccessful");
					wStatusAtr = WFS_IDC_DATASRCMISSING;

				}
				if (ec == 0x45) //No card in the reader
				{
					Xfs::getInstance()->l.debug("No card in the reader");
					wStatusAtr = WFS_IDC_DATASRCMISSING;
				}
				if (ec == 0x57) //Card is no on the right position
				{
					Xfs::getInstance()->l.debug("Card is no on the right position");
					wStatusAtr = WFS_IDC_DATASRCMISSING;
			...
			if (!lenAtr)
			{
				Xfs::getInstance()->l.debug("Data Length=0");
	...
	if ((*lpwReadData)&WFS_IDC_TRACK1)
	{
		Xfs::getInstance()->l.debug("Track 1 data",data1,len1);
		...
		if (data1[1] == 0X59) {// read data correclty
			...
			Xfs::getInstance()->l.debug("Track 1 data read correctly");
	...
	if ((*lpwReadData)&WFS_IDC_TRACK2)
	{
		Xfs::getInstance()->l.debug("Track 2 data", data2, len2);
		lppCardData[c]->ulDataLength = 0;
		...
		if (data2[1] == 0X59) {// read data correclty
			...
			Xfs::getInstance()->l.debug("Track 2 data read correctly");
	...
	if ((*lpwReadData)&WFS_IDC_TRACK3)
	{
		Xfs::getInstance()->l.debug("Track 3 data", data3, len3);
		lppCardData[c]->ulDataLength = 0;
		...
		if (data3[1] == 0X59) {// read data correclty
			...
			Xfs::getInstance()->l.debug("Track 3 data read correctly");
...
{	
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_CHIP_POWER reqId=" + std::to_string(reqId));
	...
	{
		requests[reqId]->pResult->hResult = WFS_ERR_IDC_MEDIAJAM;
		Xfs::getInstance()->l.debug("Unsuccessful = WFS_ERR_IDC_MEDIAJAM");
		return;
	}
	if (status.fwMedia != WFS_IDC_MEDIAPRESENT)
	{
		requests[reqId]->pResult->hResult = WFS_ERR_IDC_NOMEDIA;	
		Xfs::getInstance()->l.debug("Unsuccessful = WFS_ERR_IDC_NOMEDIA");
	...
	if (ec)
	{
		Xfs::getInstance()->l.debug("Error happened ec=" + std::to_string(ec));
	...
	Xfs::getInstance()->l.debug("Looks like everything is ok, creatinf output result");
...
void IDCCRT310::execWFS_CMD_IDC_CHIP_IO(REQUESTID reqId)
{
	...
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_CHIP_IO reqId=" + std::to_string(reqId));

	Xfs::getInstance()->l.debug("lpChipIoIn", lpChipIoIn->lpbChipData, lpChipIoIn->ulChipDataLength);
	...
	if (status.fwMedia == WFS_IDC_MEDIANOTPRESENT)
	{
		Xfs::getInstance()->l.debug("Media not present, ...");
		requests[reqId]->pResult->hResult = WFS_ERR_IDC_NOMEDIA;
		return;
	}
	...
	if (ec)
	{
		Xfs::getInstance()->l.debug("CAPU  sending returns an ERROR="+std::to_string(ec));
		if (ec == 0x4e){//Failure
			
			Xfs::getInstance()->l.debug("Failure");
			requests[reqId]->pResult->hResult = WFS_ERR_IDC_INVALIDDATA;			
			return;
		} 
		if (ec == 0x45){// No card in the reader
		...
		if (ec == 0x57){// Card on un-operative position
			Xfs::getInstance()->l.debug("Card on un-operative position");
	...
	if (!len)
	{
		Xfs::getInstance()->l.debug("len=0, obviously something went wrong");
	...
	Xfs::getInstance()->l.debug("lpChipIoOut", lpChipIoOut->lpbChipData, lpChipIoOut->ulChipDataLength);
...
{
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_RETAIN_CARD reqId=" + std::to_string(reqId) );
	...
	if (status.fwMedia == WFS_IDC_MEDIANOTPRESENT)
	{
		Xfs::getInstance()->l.debug("Media not present, nothing to retain");
		requests[reqId]->pResult->hResult = WFS_ERR_IDC_NOMEDIA;			
		return;
	}
	else
	{
		if (status.usCards >= caps.usCards)
		{
			...
			Xfs::getInstance()->l.debug("Mediapresent, but retain BIN is full");
...
{
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_EJECT_CARD reqId=" + std::to_string(reqId));
	...
	if (status.fwMedia == WFS_IDC_MEDIANOTPRESENT)
	{
		Xfs::getInstance()->l.debug("Media not present, nothing to eject");
		requests[reqId]->pResult->hResult = WFS_ERR_IDC_NOMEDIA;
		return;
	}
	else
...
{
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_RESET_COUNT reqId=" + std::to_string(reqId));
...
{
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_RESET reqId=" + std::to_string(reqId));
	BYTE eject = 0;
...
{
	Xfs::getInstance()->l.debug("execWFS_CMD_IDC_SET_GUIDANCE_LIGHT reqId=" + std::to_string(reqId));
	...
	Xfs::getInstance()->l.debug("Command received LIGHT = "+std::to_string(lpSetGuidLight->dwCommand));

	if (lpSetGuidLight->dwCommand == 640)
	{
		Xfs::getInstance()->l.debug("Command received LIGHT ON" );

		on = 1;
	}
	else
	{
		Xfs::getInstance()->l.debug("Command received LIGHT OFF");
		on = 0;
	}
	...
	{
		Xfs::getInstance()->l.debug("Guidlight not avaialabe id=" + std::to_string(lpSetGuidLight->wGuidLight));
	...
	*/
	...
	{
		GetErrCode(&ec);
		Xfs::getInstance()->l.debug("Error executing ledset=" + std::to_string(ec));
		checkEc(ec);
		requests[reqId]->pResult->hResult = WFS_ERR_HARDWARE_ERROR;
		return;
	}
	...
	*/
	...
	{
		onoff = 0;
		Xfs::getInstance()->l.debug("SET LIHGT ON!!!!!");
		t1 = 0xff;
		t2 = 0;
	}
	else
	{
		onoff = 1;
		Xfs::getInstance()->l.debug("SET LIHGT OFFFFFFF!!!!!");
		t1 = 0;
		t2 = 0;
	}
	...
	{
		GetErrCode(&ec);
		Xfs::getInstance()->l.debug("Error executing ledtime=" + std::to_string(ec));
		checkEc(ec);
		requests[reqId]->pResult->hResult = WFS_ERR_HARDWARE_ERROR;
		return;
	}
...
void IDCCRT310::checkEc(int ec)
{
	...
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec)+" (Card Type error)");
	}
	if (ec == - 101)// Serial port error == > Execute CommOpen or CommOpenWithBaut function to configure
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Serial port error == > Execute CommOpen or CommOpenWithBaut function to configure)");
	}
	if (ec == - 102)// Baud Rate setting error == > Execute CommOpenWithBaut to open serial port
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Execute CommOpenWithBaut to open serial port)");
	}
	if (ec == - 200)// 0 Undefined parameter
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Undefined parameter)");
	}
	if (ec == - 202)// 1 Command out of model == > Check if command support by the model number
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Command out of model == > Check if command support by the model number)");
	}
	if (ec == - 204)// 2 Command out of order == > Wrong command
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Command out of order == > Wrong command)");
	}
	if (ec == - 205)// 3 Power error == > Check power outputDC12V
	{
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Power error == > Check power outputDC12V)");
	}
	if (ec == - 206)// 4 Over - short or long card inside
	{
		this->status.fwDevice = WFS_IDC_DEVNODEVICE;
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Over - short or long card inside)");
	}
	if (ec == - 207)// 5 Power off == > Check the power
	{
		this->status.fwDevice = WFS_IDC_DEVNODEVICE;
		Xfs::getInstance()->l.debug("Error code=" + std::to_string(ec) + " (Power off == > Check the power)");
	}
...
{	
	...
	Xfs::getInstance()->l.debug("Updating device status (card position/settings)");
	...
	if (ec)
	{
		Xfs::getInstance()->l.debug("Error, ec="+std::to_string(ec));
	...
	Xfs::getInstance()->l.debug("Card position device status (card position):");
	Xfs::getInstance()->l.debug("atPosition   = " + std::to_string(atPosition)+" ("+sAtPosition+")");
	Xfs::getInstance()->l.debug("frontSetting = " + std::to_string(frontSetting) + " (" + sFrontSetting + ")");
	Xfs::getInstance()->l.debug("rearSetting  = " + std::to_string(rearSetting) + " (" + sRearSetting + ")");
